...
def get_prediction(team = None):
    ...
    df_all = df[(df['win'] == team) | (df['lose'] == team)] 


    df_win = df_all[df_all["win"] == team] # 获胜概率大于5成
    df_lose = df_all[df_all["win"] != team] # 获胜概率小于5成
    ...
    plt.title('18-19赛季{}获胜概率预测'.format(team), fontproperties=myfont)

    x_axis = [i for i in range(1, len(x) + 1)]
    plt.plot(x_axis, y)
    plt.xticks(x_axis, x, rotation=90)
    plt.savefig('./{}.png'.format(team))
    plt.grid(True) # 绘制网格
    plt.show()

if __name__ == "__main__":
    team = input('请输入需要预测的球队全名,如: Los Angeles Lakers/Cleveland Cavaliers')
    get_prediction(team=team)
